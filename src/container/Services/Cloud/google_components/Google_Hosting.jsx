...
function Google_Hosting() {
  ...
  return (
    ...
    <Heading>
      <div>
        <p className="text-[50px] text-center font-bold">Type of Google Cloud Hosting</p>
      </div>
    </Heading>
      <div className="ml-20 mr-20  flex items-center justify-between p-10">
        ...
        <div className="w-1/2  relative">
          {services.map((service, index) => (
            ...
            >
              <p className="border-l-4 border-[#0B57D0] pl-3 text-3xl font-semibold mb-10 cursor-pointer">
                {service.name}
              </p>

              {/* Hover Card - Inside the Line */}
              {hovered === index && (
                <div className="absolute left-[300px] top-1/2 transform -translate-y-1/2 w-96 bg-white p-5 shadow-lg rounded-lg border border-gray-300 z-10">
                  <p className="font-semibold text-[#0B57D0] text-xl">{service.name}</p>
        ...
        <div className="">
          <img src={Google_hosting} alt="Google Cloud Hosting" className="w-[600px] h-auto" />
        </div>
      </div>
      </WrapperContainer>
    </>
